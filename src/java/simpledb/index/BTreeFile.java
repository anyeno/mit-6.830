...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        Iterator<BTreeEntry> iterator = new BTreeInternalPageIterator(
                (BTreeInternalPage) this.getPage(tid, dirtypages, pid, perm));
        while(iterator.hasNext()) {
            BTreeEntry bTreeEntry = iterator.next();
            if(f == null) {
                return this.findLeafPage(tid, dirtypages, bTreeEntry.getLeftChild(), Permissions.READ_ONLY, f);
            }
            if(f.compare(Op.LESS_THAN_OR_EQ, bTreeEntry.getKey())) {
                return this.findLeafPage(tid, dirtypages, bTreeEntry.getLeftChild(), Permissions.READ_ONLY, f);
            } else if(f.compare(Op.GREATER_THAN, bTreeEntry.getKey())) {
                return this.findLeafPage(tid, dirtypages, bTreeEntry.getRightChild(), Permissions.READ_ONLY, f);
            }
        }
        return null;
    }
